...
function exitLogin() {
    $.ajax({
        //接口地址
        url:'http://localhost:8081/ework/admin/quit',
        //请求方式post/get
        type:'post',
...
function logout(){
    layui.use('layer', function(){
        ...
        layer.confirm('您即将注销账号，此操作不可恢复，您确认要注销账号吗？', {
            ...
            $.ajax({
                //接口地址
                url:'http://localhost:8081/ework/admin/logout',
                //请求方式post/get
                type:'post',
                contentType:'application/json',
                //数据
...
function submitChangeInfo(){
    layui.use('layer',function(){
        ...
        if(layuiNickname===''||layuiPhone===''){
            ...
            $.ajax({
                //接口地址
                url:'http://localhost:8081/ework/admin/changeDetail',
                //请求方式post/get
                type:'post',
                contentType:'application/json',
                //数据
         ...
         $.ajax({
            ...
            success:function (data) {
                if(data.code===1)
                {
                    ...
                    $('#lay-adm-phone').append(data.data.phone);
                    $('#lay-adm-group').append(data.data.adminGroupCount);
                    $('#lay-adm-create-group').append(data.data.createGroupCount);
                }
            },
            //失败的回调函数
            error:function (e) {
